...
export default function UserProfileScreen() {
  ...
  const fetchAll = async () => {
    ...
    const [profileRes, followersRes, followingRes, followStatusRes] = await Promise.all([
      supabase.from('profiles').select('*').eq('id', id).single(),
      supabase.from('followers').select('*', { count: 'exact' }).eq('following_id', id).eq('status', 'accepted'),
      supabase.from('followers').select('*', { count: 'exact' }).eq('follower_id', id).eq('status', 'accepted'),
      supabase.from('followers').select('status').eq('follower_id', user.id).eq('following_id', id).single(),
    ]);
  ...
  const canSeePins = !profile?.is_private || isOwnProfile || followStatus === 'accepted';
              ...
              <View>
                ...
                {!isOwnProfile && (
                  <TouchableOpacity
                    style={[
                      styles.followButton,
                      (followStatus === 'accepted' || followStatus === 'pending') && styles.followingButton,
                ...
                {canSeePins ? (
                  ...
                  <View style={styles.privateNotice}>
                    ...
                    <Text style={styles.privateNoticeText}>This account is private</Text>
...
const styles = StyleSheet.create({
  ...
  followButton: { marginHorizontal: 16, marginBottom: 16, paddingVertical: 10, borderRadius: 10, backgroundColor: '#2D6A4F', alignItems: 'center' },
  followButtonText: { fontSize: 15, fontWeight: '600', color: '#fff' },
  followingButton: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#2D6A4F' },
  ...
  privateNotice: { alignItems: 'center', paddingVertical: 48, paddingHorizontal: 32, gap: 8 },
  privateNoticeEmoji: { fontSize: 40 },
  privateNoticeText: { fontSize: 17, fontWeight: '700', color: '#11181C' },
  privateNoticeSub: { fontSize: 14, color: '#687076', textAlign: 'center', lineHeight: 20 },
